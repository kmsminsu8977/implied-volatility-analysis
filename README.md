...
내재변동성 분석 프로젝트의 기본 연구 구조와 재현 가능한 baseline 산출물을 담은 저장소입니다.
...
- 관측 가격을 입력으로 두고 이분법으로 Black-Scholes 내재변동성을 찾는다.
- moneyness 구간별로 변동성 차이를 비교해 smile/skew의 직관을 정리한다.
- 가격 데이터는 행사가별 smile 구조를 드러내도록 구성한 학습용 quote다.

## 주요 파일

- `src/implied_volatility_baseline.py`: 시장 옵션가격에서 Black-Scholes 내재변동성을 역산하고 smile 구조를 해석한다.
- `data/sample/market_option_quotes.csv`: baseline 실행용 합성 입력값
