...
이 문서는 `implied-volatility-analysis` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 내재변동성 분석 (Implied Volatility Analysis)
- 연구 카테고리: Derivatives Pricing & Volatility
- 핵심 질문: 같은 만기에서 행사가별 시장가격은 어떤 변동성 구조를 암시하는가?
...
- KAIST-DFMBA/monte-carlo-pricing-simulation/docs/source_inventory.md

## 참고 초점

- 옵션 가격과 변동성 입력값의 관계
- Black-Scholes 가격 함수를 역으로 사용하는 계산 흐름
- 행사가별 moneyness와 smile/skew 해석 순서
